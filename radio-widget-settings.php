<?php
class PBP_Radio_Widget_Settings {
    private $options;
    ...
    public function add_plugin_page() {
        add_options_page(
            'PBP Radio Stations', 
            'PBP Radio Widget Options', 
            'manage_options', 
            'PBP-radio-widget', 
            array( $this, 'pbp_options_page' )
        );
    }

    public function pbp_options_page() {
		$settings_url = admin_url( 'options-general.php' ) . '?page=PBP-radio-widget';

    	if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( __( 'You do not have sufficient permissions to access this page.', 'PBP-radio-widget' ) );
		}

		echo '<div class="wrap">'
             .'<h2><div id="icon-options-general" class="icon32"></div>'
             .__( 'PBP Radio Widget Stations', 'PBP-radio-widget' ).'</h2>';
        $radio = get_option( 'pbp_radio_settings' );
        $radio = maybe_unserialize( $radio );

        if ( isset( $_POST[ 'pbp_add_new' ] ) && !isset( $_POST[ 'station_id' ] ) ){
			if ( !isset( $_POST[ 'station_name' ] ) || empty( $_POST[ 'station_name' ] )
            || !isset( $_POST[ 'station_url' ] ) || empty( $_POST[ 'station_url' ] ) ) {
				echo '<div id="message" class="error"><p>'. __( 'Please fill all the boxes!', 'PBP-radio-widget' ) .'</p></div>';
			} else {
                $tmp = array();
                ...
                $radio[ 'stations' ][] = $tmp;
                var_dump("boalfmoa");
                update_option( 'pbp_radio_settings', $radio );
			}
		}
		    ...
		    if ( !isset( $_POST[ 'station_name' ] ) || empty( $_POST[ 'station_name' ] ) 
                || !isset( $_POST[ 'station_url' ] ) || empty( $_POST[ 'station_url' ] ) ) {
			        echo '<div id="message" class="error"><p>'
                            . __( 'Please fill all the boxes!', 'PBP-radio-widget' ) .'</p></div>';
		    } else {
			    $tmp = array();
                $tmp[ 'name' ] = $_POST[ 'station_name' ];
                $tmp[ 'url' ] = $_POST[ 'station_url' ];
        ...
        if ( isset( $_GET[ 'action' ] ) ) {
			if ( !isset( $_GET[ 'id' ] ) || empty( $_GET[ 'id' ] ) ) {
				echo '<div id="message" class="error"><p>'. __( 'There was an error!', 'PBP-radio-widget' ) .'</p></div>';
		...
		echo '<table class="widefat">
				<thead>
				    <tr>
                        <th>'. __( 'ID', 'PBP-radio-widget' ).'</th>
				        <th>'. __( 'Name', 'PBP-radio-widget' ).'</th>
				        <th>'. __( 'Url', 'PBP-radio-widget' ).'</th>
                        <th>'. __( 'Action', 'PBP-radio-widget' ).'</th>
				    </tr>
				</thead>
				<tfoot>
				    <tr>
			...
			echo '<tr>
				     ...
				     <td>
                        <a class="edit_link" href="'. $settings_url .'&action=edit&id='. $id .'">'
                            . __( 'Edit', 'PBP-radio-widget' ) .'</a> | 
                        <a class="delete_link" href="'. $settings_url .'&action=delete&id='. $id .'">'
                            . __( 'Delete', 'PBP-radio-widget' ) .'</a></td>
		   		</tr>';
            $id++;
		}
        ...
        if ($flag) {
            echo '<td>'. __( 'Editing', 'PBP-radio-widget') .'</td>';
        } else {
            echo '<td>'. __( 'New', 'PBP-radio-widget') .'</td>';
        }
        ...
        echo '" id="station_url" name="station_url" /></td>
              <td><input name="save" type="submit" class="button-primary" value="'. __( 'Save', 'PBP-radio-widget' ) .'" />';
        if ($flag) {
            echo '<input type="hidden" name="station_id" value="'. $station_to_edit[ 'id' ] .'" />';
        }
        echo '<input type="hidden" name="pbp_add_new" value="1" /></td>
              </tbody></table></div>';
        echo '</div>';
	}
}
